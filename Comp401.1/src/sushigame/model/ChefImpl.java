...
public class ChefImpl implements Chef, BeltObserver {
	...
	private double decTotalFoodSpoiledCount;
	private double decTotalFoodConsumedCount;
	...
	public void handleBeltEvent(BeltEvent e) {
		if (e.getType() == BeltEvent.EventType.PLATE_CONSUMED) {
			...
			if (plate.getChef() == this) {
				decTotalFoodConsumedCount += Amount(plate);
			...
			if(plate.getChef() == this){
				decTotalFoodSpoiledCount += Amount(plate);
			}
	...
	@Override
	public double getFoodSpoiledCount() {
		return decTotalFoodSpoiledCount;
	}

	@Override
	public double getFoodConsumedCount() {
		return decTotalFoodConsumedCount;
	}
	public double Amount(Plate p) {
		double totalCount = 0;
		Sushi tester = p.getContents();
