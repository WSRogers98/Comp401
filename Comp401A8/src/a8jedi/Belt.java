...
public class Belt extends Observable {
	...
	public void rotate() {
		...
		for(int i = 0; i <belt.length; i ++) {
			if(belt[i]!= null) {
				if(belt[i].getContents().getHasShellfish() == true && belt[i].getAge()  >= this.getSize() * 1) {
					setChanged();
				notifyObservers(new PlateEvent(PlateEvent.EventType.PLATE_SPOILED, belt[i].getPlate(),  i));
				}else
				if(belt[i].getContents().getIsVegetarian() == true && belt[i].getAge()  >= this.getSize() * 3) {
					setChanged();
				notifyObservers(new PlateEvent(PlateEvent.EventType.PLATE_SPOILED, belt[i].getPlate(),  i));
				}else
					if(belt[i].getContents().getIsVegetarian() != true && belt[i].getContents().getHasShellfish() !=true && belt[i].getAge() >= this.getSize() * 2) {
						setChanged();
						notifyObservers(new PlateEvent(PlateEvent.EventType.PLATE_SPOILED, belt[i].getPlate(),  i));
					}			
			}
